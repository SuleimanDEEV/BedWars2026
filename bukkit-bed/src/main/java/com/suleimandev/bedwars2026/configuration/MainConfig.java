...
public class MainConfig extends ConfigManager {
    ...
    public MainConfig(Plugin plugin, String name) {
        super(plugin, name, BukkitBedPlugin.plugin.getDataFolder().getPath());

        YamlConfiguration yml = getYml();

        yml.options().header(plugin.getDescription().getName() + " by SuleimanDEV.\n" +
                "Internal build and configuration generated for BEDWARS 2026.\n");
        ...
        yml.addDefault("storeLink", "https://example.com/bedwars2026");
        ...
        yml.addDefault(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_POWERED_BY, "BEDWARS 2026");
        ...
        yml.addDefault(ConfigPath.GENERAL_TNT_PROTECTION_END_STONE_BLAST, BukkitBedPlugin.nms.getVersion() == 0 ? 69f : 12f);
        ...
        yml.addDefault("database.database", "bedwars2026");
            ...
            for (File f : langs) {
                if (f.isFile()) {
                    if (f.getName().contains("messages_") && f.getName().contains(".yml")) {
                        ...
                        if (Language.getLang(lang) == null) new Language(BukkitBedPlugin.plugin, lang);
                    }
                }
            }
        }
        ...
        BukkitBedPlugin.setDebug(yml.getBoolean("debug"));
        ...
        try {
            BukkitBedPlugin.setServerType(ServerType.valueOf(Objects.requireNonNull(yml.getString("serverType")).toUpperCase()));
        } catch (Exception e) {
            if (Objects.requireNonNull(yml.getString("serverType")).equalsIgnoreCase("BUNGEE_LEGACY")) {
                BukkitBedPlugin.setServerType(ServerType.BUNGEE);
                BukkitBedPlugin.setAutoscale(false);
            } else {
                set("serverType", "MULTIARENA");
            }
        }

        BukkitBedPlugin.setLobbyWorld(getLobbyWorldName());
    }
