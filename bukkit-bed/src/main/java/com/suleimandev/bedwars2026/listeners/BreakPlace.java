...
import com.suleimandev.bedwars2026.api.util.BlastProtectionUtil;
...
import com.suleimandev.bedwars2026.popuptower.TowerEast;
import com.suleimandev.bedwars2026.popuptower.TowerNorth;
import com.suleimandev.bedwars2026.popuptower.TowerSouth;
import com.suleimandev.bedwars2026.popuptower.TowerWest;
...
public class BreakPlace implements Listener {
    ...
    public BreakPlace() {
        ...
        blastProtection = new BlastProtectionUtil(nms, BukkitBedPlugin.getAPI());
    }

    @EventHandler
    public void onIceMelt(BlockFadeEvent e) {
        if (BukkitBedPlugin.getServerType() == ServerType.MULTIARENA) {
            if (Objects.requireNonNull(e.getBlock().getLocation().getWorld()).getName().equalsIgnoreCase(BukkitBedPlugin.getLobbyWorld())) {
                e.setCancelled(true);
                return;
            }
        }
    ...
    public void onBlockPlace(BlockPlaceEvent e) {
        ...
        if (a != null) {
            ...
            } else if (BukkitBedPlugin.shop.getBoolean(ConfigPath.SHOP_SPECIAL_TOWER_ENABLE)) {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (BukkitBedPlugin.getServerType() == ServerType.MULTIARENA
                && player.getWorld().getName().equalsIgnoreCase(BukkitBedPlugin.getLobbyWorld())) {
    ...
    public void onSignChange(SignChangeEvent e) {
        ...
        if (Objects.requireNonNull(e.getLine(0)).equalsIgnoreCase("[" + mainCmd + "]")) {
            ...
            if (dir.exists()) {
                ...
                if (a != null) {
                    p.sendMessage("Â§aâ–ª Â§7Sign saved for arena: " + e.getLine(1));
                ...
                p.sendMessage("Â§câ–ª Â§7You didn't set any arena yet!");
            }
        }
    }
    ...
    public void onBucketFill(PlayerBucketFillEvent e) {
        if (e.isCancelled()) return;
        if (BukkitBedPlugin.getServerType() == ServerType.MULTIARENA) {
            if (Objects.requireNonNull(e.getPlayer().getLocation().getWorld()).getName().equalsIgnoreCase(BukkitBedPlugin.getLobbyWorld())) {
    ...
    public void onPaintingRemove(HangingBreakByEntityEvent e) {
        ...
        if (a == null) {
            if (BukkitBedPlugin.getServerType() == ServerType.SHARED) return;
            if (!BukkitBedPlugin.getLobbyWorld().equals(e.getEntity().getWorld().getName())) return;
        }
    ...
    public void soilChangeEntity(EntityChangeBlockEvent e) {
        if (e.getTo() == Material.DIRT) {
            if (e.getBlock().getType().toString().equals("FARMLAND") || e.getBlock().getType().toString().equals("SOIL")) {
                if ((Arena.getArenaByIdentifier(e.getBlock().getWorld().getName()) != null) || (e.getBlock().getWorld().getName().equals(BukkitBedPlugin.getLobbyWorld())))
                    e.setCancelled(true);
            }
        }
    }
