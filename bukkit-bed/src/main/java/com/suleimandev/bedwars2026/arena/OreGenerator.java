...
import com.suleimandev.bedwars2026.api.arena.generator.IGenHolo;
...
import com.suleimandev.bedwars2026.api.events.gameplay.GeneratorUpgradeEvent;
...
public class OreGenerator implements IGenerator {
    ...
    public OreGenerator(Location location, IArena arena, GeneratorType type, ITeam bwt) {
        ...
        this.type = type;
        loadDefaults();
        BukkitBedPlugin.debug("Initializing new generator at: " + location + " - " + type + " - " + (bwt == null ? "NOTEAM" : bwt.getName()));
        ...
        if (lastSpawn == 0) {
            ...
            if (plugin.getConfig().getBoolean(ConfigPath.GENERAL_CONFIGURATION_ENABLE_GEN_SPLIT)) {
                ...
                for (Object o : players) {
                    ...
                    player.playSound(player.getLocation(), Sound.valueOf(BukkitBedPlugin.getForCurrentVersion("ITEM_PICKUP", "ENTITY_ITEM_PICKUP", "ENTITY_ITEM_PICKUP")), 0.6f, 1.3f);
    ...
    public void setOre(ItemStack ore) {
        BukkitBedPlugin.debug("Changing ore for generator at " + location.toString() + " from " + this.ore + " to " + ore);
        this.ore = ore;
    }
