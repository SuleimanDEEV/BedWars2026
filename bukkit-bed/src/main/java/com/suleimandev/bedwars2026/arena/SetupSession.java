...
import com.suleimandev.bedwars2026.api.events.server.SetupSessionCloseEvent;
import com.suleimandev.bedwars2026.api.events.server.SetupSessionStartEvent;
...
public class SetupSession implements ISetupSession {
    ...
    public static String getInvName() {
        return "Ã‚Â§8Choose a setup method";
    }
    ...
    public boolean startSetup() {
        getPlayer().sendMessage("Ã‚Â§6 Ã¢â€“Âª Ã‚Â§7Loading " + getWorldName());
        cm = new ArenaConfig(BukkitBedPlugin.plugin, getWorldName(), plugin.getDataFolder().getPath() + "/Arenas");
        BukkitBedPlugin.getAPI().getRestoreAdapter().onSetupSessionStart(this);
        return true;
    }
    ...
    private static void openGUI(Player player) {
        ...
        am.setDisplayName("Ã‚Â§eÃ‚Â§lASSISTED SETUP");
        am.setLore(Arrays.asList("", "Ã‚Â§aEasy and quick setup!", "Ã‚Â§7For beginners and lazy staff :D", "", "Ã‚Â§3Reduced options."));
        ...
        amm.setDisplayName("Ã‚Â§cÃ‚Â§lADVANCED SETUP");
        amm.setLore(Arrays.asList("", "Ã‚Â§aDetailed setup!", "Ã‚Â§7For experienced staff :D", "", "Ã‚Â§3Advanced options."));
    ...
    public void cancel() {
        ...
        if (isStarted()) {
            player.sendMessage("Ã‚Â§6 Ã¢â€“Âª Ã‚Â§7" + getWorldName() + " setup cancelled!");
            done();
        }
    }
     ...
     */
    public void done() {
        BukkitBedPlugin.getAPI().getRestoreAdapter().onSetupSessionClose(this);
        getSetupSessions().remove(this);
        ...
        if (BukkitBedPlugin.getServerType() == ServerType.MULTIARENA) Arena.sendLobbyCommandItems(getPlayer());
    ...
    public void teleportPlayer() {
        ...
        if (getSetupType() == SetupType.ASSISTED && getConfig().getYml().get("waiting.Loc") == null) {
            ...
            player.spigot().sendMessage(Misc.msgHoverClick(ChatColor.BLUE + "     Ã¢â€“Âª     " + ChatColor.GOLD + "CLICK HERE TO SET THE WAITING LOBBY    " + ChatColor.BLUE + " Ã¢â€“Âª", ChatColor.LIGHT_PURPLE + "Click to set the waiting spawn.", "/" + BukkitBedPlugin.mainCmd + " setWaitingSpawn", ClickEvent.Action.RUN_COMMAND));
            player.spigot().sendMessage(MainCommand.createTC(ChatColor.YELLOW + "Or type: " + ChatColor.GRAY + "/" + BukkitBedPlugin.mainCmd + " to see the command list.", "/" + BukkitBedPlugin.mainCmd + "", ChatColor.WHITE + "Show commands list."));
        } else {
            Bukkit.dispatchCommand(player, BukkitBedPlugin.mainCmd + " cmds");
        }
    ...
    public String dot() {
        return ChatColor.BLUE + " " + '*' + " " + ChatColor.GRAY + "/" + BukkitBedPlugin.mainCmd + " ";
    }
