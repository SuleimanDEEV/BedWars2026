...
public class GamePlayingTask implements Runnable, PlayingTask {
    ...
    public GamePlayingTask(Arena arena) {
        this.arena = arena;
        this.beds_destroy_countdown = BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_BEDS_DESTROY_COUNTDOWN);
        this.dragon_spawn_countdown = BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_DRAGON_SPAWN_COUNTDOWN);
        this.game_end_countdown = BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_GAME_END_COUNTDOWN);
        this.task = Bukkit.getScheduler().runTaskTimer(BukkitBedPlugin.plugin, this, 0, 20L);
    }
    ...
    public void run() {
            ...
            if (t.getSize() > 1) {
                for (Player p : t.getMembers()) {
                    ...
                    nms.playAction(p, getMsg(p, Messages.FORMATTING_ACTION_BAR_TRACKING).replace("{team}", t.getColor().chat() + t.getDisplayName(Language.getPlayerLanguage(p)))
                            .replace("{distance}", t.getColor().chat().toString() + distance).replace("&", "Â§"));
                }
            }
        ...
        for (Player p : getArena().getPlayers()) {
            if (Arena.afkCheck.get(p.getUniqueId()) == null) {
                ...
                if (current == 45) {
                    BukkitBedPlugin.getAPI().getAFKUtil().setPlayerAFK(p, true);
                }
            }
        }
