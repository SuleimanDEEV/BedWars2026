...
import com.suleimandev.bedwars2026.api.exceptions.InvalidMaterialException;
...
public class Misc {
    ...
    public static String replaceStatsPlaceholders(Player player, @NotNull String s, boolean papiReplacements) {
        PlayerStats stats = BukkitBedPlugin.getStatsManager().get(player.getUniqueId());
        ...
        if (s.contains("{prefix}")) s = s.replace("{prefix}", BukkitBedPlugin.getChatSupport().getPrefix(player));
