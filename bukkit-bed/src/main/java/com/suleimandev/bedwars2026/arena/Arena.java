...
import com.suleimandev.bedwars2026.api.arena.team.ITeamAssigner;
...
import com.suleimandev.bedwars2026.api.events.gameplay.NextEventChangeEvent;
...
import com.suleimandev.bedwars2026.api.tasks.PlayingTask;
import com.suleimandev.bedwars2026.api.tasks.RestartingTask;
import com.suleimandev.bedwars2026.api.tasks.StartingTask;
import com.suleimandev.bedwars2026.arena.stats.GameStatsManager;
import com.suleimandev.bedwars2026.arena.stats.StatisticsOrdered;
import com.suleimandev.bedwars2026.arena.tasks.GamePlayingTask;
import com.suleimandev.bedwars2026.arena.tasks.GameRestartingTask;
import com.suleimandev.bedwars2026.arena.tasks.GameStartingTask;
import com.suleimandev.bedwars2026.arena.tasks.ReJoinTask;
import com.suleimandev.bedwars2026.arena.team.BedWarsTeam;
import com.suleimandev.bedwars2026.arena.team.TeamAssigner;
...
import com.suleimandev.bedwars2026.levels.internal.PerMinuteTask;
import com.suleimandev.bedwars2026.listeners.blockstatus.BlockStatusListener;
import com.suleimandev.bedwars2026.listeners.dropshandler.PlayerDrops;
import com.suleimandev.bedwars2026.money.internal.MoneyPerMinuteTask;
import com.suleimandev.bedwars2026.shop.ShopCache;
import com.suleimandev.bedwars2026.sidebar.BwSidebar;
...
import com.suleimandev.bedwars2026.support.vault.WithEconomy;
...
import static com.suleimandev.bedwars2026.arena.upgrades.BaseListener.isOnABase;
...
public class Arena implements IArena {
    ...
    public Arena(String name, Player p) {
        ...
        this.arenaName = name;
        if (autoscale) {
            this.worldName = BukkitBedPlugin.matchIdGenerator.generateGameID();
        } else {
            this.worldName = arenaName;
        }

        cm = new ArenaConfig(BukkitBedPlugin.plugin, name, plugin.getDataFolder().getPath() + "/Arenas");
        ...
        if (yml.getConfigurationSection("Team").getKeys(false).size() < 2) {
            if (p != null) p.sendMessage("Ã‚Â§cYou must set at least 2 teams on: " + name);
        ...
        if (!BukkitBedPlugin.getAPI().getRestoreAdapter().isWorld(name)) {
        ...
        for (String team : yml.getConfigurationSection("Team").getKeys(false)) {
            ...
            try {
                ...
                if (p != null) p.sendMessage("Ã‚Â§cInvalid color at team: " + team + " in arena: " + name);
            ...
            for (String stuff : Arrays.asList("Color", "Spawn", "Bed", "Shop", "Upgrade", "Iron", "Gold")) {
                if (yml.get("Team." + team + "." + stuff) == null) {
                    if (p != null) p.sendMessage("Ã‚Â§c" + stuff + " not set for " + team + " team on: " + name);
        ...
        if (yml.get("generator.Diamond") == null) {
            if (p != null) p.sendMessage("Ã‚Â§cThere isn't set any Diamond generator on: " + name);
            plugin.getLogger().severe("There isn't set any Diamond generator on: " + name);
        }
        if (yml.get("generator.Emerald") == null) {
            if (p != null) p.sendMessage("Ã‚Â§cThere isn't set any Emerald generator on: " + name);
            plugin.getLogger().severe("There isn't set any Emerald generator on: " + name);
        }
        if (yml.get("waiting.Loc") == null) {
            if (p != null) p.sendMessage("Ã‚Â§cWaiting spawn not set on: " + name);
    ...
    public void init(World world) {
        ...
        for (String team : yml.getConfigurationSection("Team").getKeys(false)) {
            if (getTeam(team) != null) {
                BukkitBedPlugin.plugin.getLogger().severe("A team with name: " + team + " was already loaded for arena: " + getArenaName());
                continue;
            }
        ...
        if (!getConfig().getYml().isSet(ConfigPath.ARENA_WAITING_POS1) && getConfig().getYml().isSet(ConfigPath.ARENA_WAITING_POS2)) {
            plugin.getLogger().severe("Lobby Pos1 isn'*'s lobby won't be removed!");
        }
        if (getConfig().getYml().isSet(ConfigPath.ARENA_WAITING_POS1) && !getConfig().getYml().isSet(ConfigPath.ARENA_WAITING_POS2)) {
            plugin.getLogger().severe("Lobby Pos2 isn'*'s lobby won't be removed!");
        }
    ...
    public boolean addPlayer(Player p, boolean skipOwnerCheck) {
        ...
        List<Player> partyMembersToJoin = new ArrayList<>();
        partyMembersToJoin.add(p);
        if (getParty().hasParty(p)) {
            if (!skipOwnerCheck) {
                ...
                partyMembersToJoin.clear();
                for (Player member : getParty().getMembers(p)) {
                    IArena arena = Arena.getArenaByPlayer(member);
                    if (arena == null || arena.isSpectator(member)) {
                        partyMembersToJoin.add(member);
                    }
                }
                requestedSlots = Math.max(1, partyMembersToJoin.size());

                if (requestedSlots > maxInTeam * getTeams().size() - getPlayers().size()) {
        ...
        if (status == GameState.waiting || (status == GameState.starting && (startingTask != null && startingTask.getCountdown() > 1))) {
            VipReservedSlotPolicy.Result reservation = BukkitBedPlugin.vipFeatures.reserveSlots(this, p, requestedSlots);
            if (!reservation.isAllowed()) {
                if (!BukkitBedPlugin.vipFeatures.isVip(p)) {
                    ...
                    p.sendMessage(BukkitBedPlugin.vipFeatures.getDenyMessage(reservation.getDenyReason()));
                    BukkitBedPlugin.vipFeatures.playDeniedSound(p);
                }
                return false;
            }
            ...
            for (Player displaced : reservation.getDisplacedPlayers()) {
                ...
                BukkitBedPlugin.vipFeatures.logReservation("Reserved slot in arena " + getArenaName() + " for " + p.getName() + " by removing " + displaced.getName());
                removePlayer(displaced, false);
            }

            if (!skipOwnerCheck) {
                for (Player mem : partyMembersToJoin) {
                    if (mem.equals(p)) continue;
                    IArena a = Arena.getArenaByPlayer(mem);
                    if (a != null && a.isSpectator(mem)) {
                ...
                if (startingTask != null) {
                    if (Bukkit.getScheduler().isCurrentlyRunning(startingTask.getTask())) {
                        if (startingTask.getCountdown() > getConfig().getInt(ConfigPath.GENERAL_CONFIGURATION_START_COUNTDOWN_HALF)) {
                            startingTask.setCountdown(BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_START_COUNTDOWN_HALF));
                        }
                    }
                }
            }
            ...
            if (!isStatusChange) {
                MatchSidebarService.getInstance().giveSidebar(p, this, false);
            }
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            ...
            if (getServerType() == ServerType.BUNGEE) {
                ...
                //if (BukkitBedPlugin.nms.getVersion() == 7) {
                BukkitBedPlugin.nms.sendPlayerSpawnPackets(p, this);
                //}
            }
            for (Player on : Bukkit.getOnlinePlayers()) {
                ...
                if (isPlayer(on)) {
                    BukkitBedPlugin.nms.spigotShowPlayer(p, on);
                    BukkitBedPlugin.nms.spigotShowPlayer(on, p);
                } else {
                    BukkitBedPlugin.nms.spigotHidePlayer(p, on);
                    BukkitBedPlugin.nms.spigotHidePlayer(on, p);
                }
            }
        ...
        if (getPlayers().size() >= getMaxPlayers()) {
            if (startingTask != null) {
                if (Bukkit.getScheduler().isCurrentlyRunning(startingTask.getTask())) {
                    if (startingTask.getCountdown() > BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_START_COUNTDOWN_SHORTENED)) {
                        startingTask.setCountdown(BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_START_COUNTDOWN_SHORTENED));
                    }
                }
            }
        }
    ...
    public void removePlayer(@NotNull Player p, boolean disconnect) {
        ...
        BukkitBedPlugin.getAPI().getAFKUtil().setPlayerAFK(p, false);
        ...
        if (status == GameState.starting && (maxInTeam > players.size() && teamuri || players.size() < minPlayers && !teamuri)) {
            ...
            BukkitBedPlugin.debug("removePlayer debug1");
            int alive_teams = 0;
            for (ITeam t : getTeams()) {
                if (t == null) continue;
            ...
            if (alive_teams == 1 && !BukkitBedPlugin.isShuttingDown()) {
                checkWinner();
                Bukkit.getScheduler().runTaskLater(BukkitBedPlugin.plugin, () -> changeStatus(GameState.restarting), 10L);
                if (team != null) {
            ...
            } else if (alive_teams == 0 && !BukkitBedPlugin.isShuttingDown()) {
                Bukkit.getScheduler().runTaskLater(BukkitBedPlugin.plugin, () -> changeStatus(GameState.restarting), 10L);
            } else if (!BukkitBedPlugin.isShuttingDown()) {
                //ReJoin feature
        ...
        if (getServerType() == ServerType.SHARED) {
            MatchSidebarService.getInstance().remove(p);
        ...
        if (!BukkitBedPlugin.isShuttingDown()) {
            Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                ...
                if (!disconnect) MatchSidebarService.getInstance().giveSidebar(p, null, false);
            }, 5L);
        }
    ...
    public boolean reJoin(Player p) {
        ...
        PlayerReJoinEvent ev = new PlayerReJoinEvent(p, this, BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_RE_SPAWN_COUNTDOWN));
        Bukkit.getPluginManager().callEvent(ev);
        if (ev.isCancelled()) return false;
        ...
        MatchSidebarService.getInstance().giveSidebar(p, this, true);
        return true;
    }
    ...
    public void disable() {
        ...
        BukkitBedPlugin.getAPI().getRestoreAdapter().onDisable(this);
    ...
    public void restart() {
        ...
        BukkitBedPlugin.getAPI().getRestoreAdapter().onRestart(this);
        destroyData();
    }
    ...
    public void changeStatus(GameState status) {
        ...
        // Prevent duplicate leave execution.
        ...
        players.forEach(c -> MatchSidebarService.getInstance().giveSidebar(c, this, false));

        spectators.forEach(c -> MatchSidebarService.getInstance().giveSidebar(c, this, false));

        if (status == GameState.starting) {
            startingTask = new GameStartingTask(this);
        } else if (status == GameState.playing) {
            if (BukkitBedPlugin.getLevelSupport() instanceof InternalLevel) {
                perMinuteTask = new PerMinuteTask(this);
            }
            if (BukkitBedPlugin.getEconomy() instanceof WithEconomy) {
    ...
    public static boolean isVip(Player p) {
        return BukkitBedPlugin.vipFeatures != null
                ? BukkitBedPlugin.vipFeatures.isVip(p)
                : p.hasPermission(mainCmd + ".*") || p.hasPermission(mainCmd + ".vip");
    }
    ...
    public synchronized void refreshSigns() {
        for (Block b : getSigns()) {
            ...
            if (s == null) return;
            int line = 0;
            for (String string : BukkitBedPlugin.signs.getList("format")) {
    ...
    public static void sendLobbyCommandItems(Player p) {
        ...
        if (!BukkitBedPlugin.config.getLobbyWorldName().equalsIgnoreCase(p.getWorld().getName())) return;
        p.getInventory().clear();
        ...
        Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, () -> {
            ...
            for (String item : config.getYml().getConfigurationSection(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_PATH).getKeys(false)) {
                ...
                if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_MATERIAL.replace("%path%", item)) == null) {
                    BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_MATERIAL.replace("%path%", item) + " is not set!");
                    continue;
                }
                if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_DATA.replace("%path%", item)) == null) {
                    BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_DATA.replace("%path%", item) + " is not set!");
                    continue;
                }
                if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_SLOT.replace("%path%", item)) == null) {
                    BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_SLOT.replace("%path%", item) + " is not set!");
                    continue;
                }
                if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_ENCHANTED.replace("%path%", item)) == null) {
                    BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_ENCHANTED.replace("%path%", item) + " is not set!");
                    continue;
                }
                if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_COMMAND.replace("%path%", item)) == null) {
                    BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_COMMAND.replace("%path%", item) + " is not set!");
                    continue;
                }
    ...
    public void sendPreGameCommandItems(Player p) {
        ...
        for (String item : config.getYml().getConfigurationSection(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_PATH).getKeys(false)) {
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_MATERIAL.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_MATERIAL.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_DATA.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_DATA.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_SLOT.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_SLOT.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_ENCHANTED.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_ENCHANTED.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_COMMAND.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_PRE_GAME_ITEMS_COMMAND.replace("%path%", item) + " is not set!");
                continue;
            }
    ...
    public void sendSpectatorCommandItems(Player p) {
        ...
        for (String item : config.getYml().getConfigurationSection(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_PATH).getKeys(false)) {
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_MATERIAL.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_MATERIAL.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_DATA.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_DATA.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_SLOT.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_SLOT.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_ENCHANTED.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_ENCHANTED.replace("%path%", item) + " is not set!");
                continue;
            }
            if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_COMMAND.replace("%path%", item)) == null) {
                BukkitBedPlugin.plugin.getLogger().severe(ConfigPath.GENERAL_CONFIGURATION_SPECTATOR_ITEMS_COMMAND.replace("%path%", item) + " is not set!");
                continue;
            }
                ...
                if (winner != null) {
                    ...
                    if (null != topInChat) {
                        ...
                        for (Player receiver : receivers) {
                            ...
                            ISidebar sidebar = MatchSidebarService.getInstance().getSidebar(receiver);
    ...
    private void registerSigns() {
        if (getServerType() != ServerType.BUNGEE) {
            if (BukkitBedPlugin.signs.getYml().get("locations") != null) {
                for (String st : BukkitBedPlugin.signs.getYml().getStringList("locations")) {
...
//        }
    ...
    public static boolean joinRandomArena(Player p) {
        ...
        for (IArena a : arenas) {
            if ((a instanceof Arena && BukkitBedPlugin.vipFeatures.canReserveSlots((Arena) a, p, amount))
                    || a.getMaxPlayers() - a.getPlayers().size() >= amount) {
    ...
    public void destroyData() {
        ...
        for (Despawnable despawnable : new ArrayList<>(BukkitBedPlugin.nms.getDespawnablesList().values())) {
    ...
    public static void removeFromEnableQueue(IArena a) {
        ...
        if (!enableQueue.isEmpty()) {
            BukkitBedPlugin.getAPI().getRestoreAdapter().onEnable(enableQueue.get(0));
    ...
    public static void addToEnableQueue(IArena a) {
        ...
        if (enableQueue.size() == 1) {
            BukkitBedPlugin.getAPI().getRestoreAdapter().onEnable(a);
        ...
        if (respawnSessions.get(player) == null) {
            ...
            if (seconds > 1) {
                ...
                for (Player playing : arena.getPlayers()) {
                    ...
                    BukkitBedPlugin.nms.spigotHidePlayer(player, playing);
                }
                ...
                Bukkit.getScheduler().runTaskLater(BukkitBedPlugin.plugin, () -> {
                    ...
                    for (Player invisible : getShowTime().keySet()) {
                        BukkitBedPlugin.nms.hideArmor(invisible, player);
                    }
    ...
    private void sendToMainLobby(Player player) {
        if (BukkitBedPlugin.getServerType() == ServerType.SHARED) {
        ...
        } else if (BukkitBedPlugin.getServerType() == ServerType.MULTIARENA) {
            if (BukkitBedPlugin.getLobbyWorld().isEmpty()) {
                TeleportManager.teleportC(player, Bukkit.getWorlds().get(0).getSpawnLocation(), PlayerTeleportEvent.TeleportCause.PLUGIN);
                plugin.getLogger().log(Level.SEVERE, player.getName() + " was teleported to the main world because lobby location is not set!");
            } else {
