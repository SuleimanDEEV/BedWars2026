...
import com.suleimandev.bedwars2026.api.events.sidebar.PlayerSidebarInitEvent;
...
import com.suleimandev.bedwars2026.sidebar.thread.*;
...
import static com.suleimandev.bedwars2026.api.language.Language.getScoreboard;
...
public class MatchSidebarService implements ISidebarService {

    private static MatchSidebarService instance;
    ...
    public static boolean init(JavaPlugin plugin) {
        if (null == instance) {
            instance = new MatchSidebarService();
            ...
            boolean lobbySidebar = config.getBoolean(ConfigPath.SB_CONFIG_SIDEBAR_USE_LOBBY_SIDEBAR) &&
                    BukkitBedPlugin.getServerType() == ServerType.MULTIARENA;
            ...
            BukkitBedPlugin.registerEvents(new ScoreboardListener());
        }
        return instance.sidebarHandler != null;
    }

    private MatchSidebarService() {
        ...
        if (null != sidebar) {
            if (null == arena) {
                ...
                if (!config.getBoolean(ConfigPath.SB_CONFIG_SIDEBAR_USE_LOBBY_SIDEBAR) ||
                        BukkitBedPlugin.getServerType() == ServerType.SHARED) {
                    this.remove(sidebar);
                    return;
                }
            } else {
    ...
    public static MatchSidebarService getInstance() {
        return instance;
    }
