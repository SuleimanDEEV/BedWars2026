...
import com.suleimandev.bedwars2026.levels.internal.PlayerLevel;
...
import com.suleimandev.bedwars2026.sidebar.api.SidebarLineAnimated;
import com.suleimandev.bedwars2026.sidebar.api.TabHeaderFooter;
import com.suleimandev.bedwars2026.sidebar.manager.SidebarManager;
...
public class BwSidebar implements ISidebar {
    ...
    protected BwSidebar(Player player) {
        ...
        String poweredBy = BukkitBedPlugin.config.getString(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_POWERED_BY);
        ...
        String serverIp = BukkitBedPlugin.config.getString(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_SERVER_IP);
    ...
    public void setContent(List<String> titleArray, List<String> lineArray, @Nullable IArena arena) {
        ...
        if (null == handle) {
            handle = MatchSidebarService.getInstance().getSidebarHandler().createSidebar(title, lines, placeholders);
            handle.add(player);
        } else {
            handle.clearLines();
    ...
    public @NotNull LinkedList<SidebarLine> normalizeLines(@NotNull List<String> lineArray) {
        ...
        for (String line : lineArray) {
            ...
            line = line
                    .replace("{serverIp}", BukkitBedPlugin.config.getString(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_SERVER_IP))
                    .replace("{poweredBy}", BukkitBedPlugin.config.getString(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_POWERED_BY))
    ...
    @NotNull ConcurrentLinkedQueue<PlaceholderProvider> getPlaceholders(@NotNull Player player) {
        ...
        // fixme 29/08/2023: disabled for now because this is not a dynamic placeholder. Let'*'s the impact.
//        providers.add(new PlaceholderProvider("{serverIp}", () -> BukkitBedPlugin.config.getString(ConfigPath.GENERAL_CONFIG_PLACEHOLDERS_REPLACEMENTS_SERVER_IP)));
        ...
        if (hasNoArena()) {
            ...
            PlayerStats persistentStats = BukkitBedPlugin.getStatsManager().get(player.getUniqueId());
    ...
    public SidebarHandle getHandle() {
        return handle;
    }
