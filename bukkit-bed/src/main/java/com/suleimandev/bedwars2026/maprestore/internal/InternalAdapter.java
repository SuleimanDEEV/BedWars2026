package com.suleimandev.bedwars2026.maprestore.internal;
...
import com.suleimandev.bedwars2026.api.util.FileUtil;
...
import com.suleimandev.bedwars2026.maprestore.internal.files.WorldZipper;
...
public class InternalAdapter extends RestoreAdapter {
    ...
    public static File backupFolder = new File(BukkitBedPlugin.plugin.getDataFolder() + "/Cache");
            ...
            for (File file : files) {
                if (!file.getName().equals(file.getName().toLowerCase())) {
                    ...
                    if (!file.renameTo(newName)) {
                        toRemove.add(file);
                        BukkitBedPlugin.plugin.getLogger().severe("Could not rename " + file.getName() + " to " + file.getName().toLowerCase() + "! Please do it manually!");
                    } else {
                        toAdd.add(newName);
                        toRemove.add(file);
                    }
                    ...
                    if (folder.exists()) {
                        if (!folder.getName().equals(folder.getName().toLowerCase())) {
                            if (!folder.renameTo(new File(plugin.getServer().getWorldContainer().getPath() + "/" + folder.getName().toLowerCase()))) {
                                BukkitBedPlugin.plugin.getLogger().severe("Could not rename " + folder.getName() + " folder to " + folder.getName().toLowerCase() + "! Please do it manually!");
                                toRemove.add(file);
                                return;
                            }
                        }
