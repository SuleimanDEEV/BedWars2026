package com.suleimandev.bedwars2026.commands.shout;
...
public class ShoutCommand extends BukkitCommand {
    ...
    public static void updateShout(Player player) {
        ...
        if (shoutCooldown.containsKey(player.getUniqueId()))
            shoutCooldown.replace(player.getUniqueId(), System.currentTimeMillis() + (BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_SHOUT_COOLDOWN) * 1000L));
        else
            shoutCooldown.put(player.getUniqueId(), System.currentTimeMillis() + (BukkitBedPlugin.config.getInt(ConfigPath.GENERAL_CONFIGURATION_SHOUT_COOLDOWN) * 1000L));
    }
