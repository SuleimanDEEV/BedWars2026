...
public class CmdList extends SubCommand {
    ...
    public CmdList(ParentCommand parent, String name) {
        ...
        setPriority(11);
        showInList(true);
        setDisplayInfo(Misc.msgHoverClick("Ã‚Â§6 Ã¢â€“Âª Ã‚Â§7/" + MainCommand.getInstance().getName() + " " + getSubCommandName() + "         Ã‚Â§8 - Ã‚Â§e view player cmds", "Ã‚Â§fView player commands.", "/" + getParent().getName() + " " + getSubCommandName(), ClickEvent.Action.RUN_COMMAND));
    }
    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        if (SetupSession.isInSetupSession(p.getUniqueId())) {
            ...
            p.spigot().sendMessage(Misc.msgHoverClick(setBed, ChatColor.WHITE + "Set a team'*'t have to specify the team name.", "/" + getParent().getName() + " setBed", ss.getSetupType() == SetupType.ASSISTED ? ClickEvent.Action.RUN_COMMAND : ClickEvent.Action.SUGGEST_COMMAND));
            p.spigot().sendMessage(Misc.msgHoverClick(setShop, ChatColor.WHITE + "Set a team'*'t have to specify the team name.\n" + ChatColor.WHITE + "It will be spawned only when the game starts.", "/" + getParent().getName() + " setShop", ss.getSetupType() == SetupType.ASSISTED ? ClickEvent.Action.RUN_COMMAND : ClickEvent.Action.SUGGEST_COMMAND));
            p.spigot().sendMessage(Misc.msgHoverClick(setUpgrade, ChatColor.WHITE + "Set a team'*'t have to specify the team name.\n" + ChatColor.WHITE + "It will be spawned only when the game starts.", "/" + getParent().getName() + " setUpgrade", ss.getSetupType() == SetupType.ASSISTED ? ClickEvent.Action.RUN_COMMAND : ClickEvent.Action.SUGGEST_COMMAND));
            if (ss.getSetupType() == SetupType.ADVANCED) {
            ...
            TextComponent credits = new TextComponent(ChatColor.BLUE + "" + ChatColor.BOLD + MainCommand.getDot() + " " + ChatColor.GOLD + plugin.getName() + " " + ChatColor.GRAY + "v" + plugin.getDescription().getVersion() + " by SuleimanDEV");
