...
public class CloneArena extends SubCommand {
    public CloneArena(ParentCommand parent, String name) {
        ...
        setDisplayInfo(Misc.msgHoverClick("Â§6 â–ª Â§7/" + getParent().getName() + " " + getSubCommandName() + " Â§6<worldName> <newName>", "Â§fClone an existing arena.",
    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        if (args.length != 2) {
            p.sendMessage("Â§câ–ª Â§7Usage: Â§o/" + getParent().getName() + " " + getSubCommandName() + " <mapName> <newArena>");
            return true;
        }
        ...
        if (BukkitBedPlugin.getAPI().getRestoreAdapter().isWorld(args[1]) && yml2.exists()) {
            p.sendMessage("Â§câ–ª Â§7" + args[1] + " already exist!");
            return true;
        }
        if (args[1].contains("+")) {
            p.sendMessage("Â§câ–ª Â§7" + args[1] + " mustn't contain this symbol: " + ChatColor.RED + "+");
            return true;
        }
        if (Arena.getArenaByName(args[0]) != null) {
            p.sendMessage("Â§câ–ª Â§7Please disable " + args[0] + " first!");
            return true;
        }
        BukkitBedPlugin.getAPI().getRestoreAdapter().cloneArena(args[0], args[1]);
        if (yml1.exists()) {
            try {
                FileUtils.copyFile(yml1, yml2, true);
            } catch (IOException e) {
                e.printStackTrace();
                p.sendMessage("Â§câ–ª Â§7An error occurred while copying the map's config. Check the console.");
            }
        }
        p.sendMessage("Â§6 â–ª Â§7Done :D.");
        return true;
    }
