package com.suleimandev.bedwars2026.database;

import com.suleimandev.bedwars2026.BukkitBedPlugin;
import com.suleimandev.bedwars2026.api.language.Language;
import com.suleimandev.bedwars2026.shop.quickbuy.QuickBuyElement;
...
public class SQLite implements Database {
    ...
    public SQLite() {
        File folder = new File(BukkitBedPlugin.plugin.getDataFolder() + "/Cache");
        if (!folder.exists()) {
            if (!folder.mkdir()) {
                BukkitBedPlugin.plugin.getLogger().severe("Could not create /Cache folder!");
            }
        }
        ...
        if (!dataFolder.exists()) {
            try {
                if (!dataFolder.createNewFile()) {
                    BukkitBedPlugin.plugin.getLogger().severe("Could not create /Cache/shop.db file!");
                }
            } catch (IOException e) {
                e.printStackTrace();
                return;
        ...
        try {
            ...
            if (e instanceof ClassNotFoundException) {
                BukkitBedPlugin.plugin.getLogger().severe("Could Not Found SQLite Driver on your system!");
            }
            e.printStackTrace();
        }
    }
        ...
        try {
            ...
            try (PreparedStatement statement = connection.prepareStatement("SELECT uuid FROM quick_buy_2 WHERE uuid = ?;")) {
                ...
                try (ResultSet rs = statement.executeQuery()) {
                    ...
                    BukkitBedPlugin.debug("UPDATE SET SLOT " + slot + " identifier " + shopPath);
    ...
    public boolean hasQuickBuy(UUID uuid) {
        try {
            ...
            try (Statement statement = connection.createStatement()) {
                try (ResultSet rs = statement.executeQuery("SELECT uuid FROM quick_buy_2 WHERE uuid = '*';")) {
                    if (rs.next()) {
                        rs.close();
                        return true;
                    }
            ...
            try (PreparedStatement pss = connection.prepareStatement("SELECT uuid from player_levels WHERE uuid = ?;")) {
                ...
                try (ResultSet rs = pss.executeQuery()) {
                    if (!rs.next()) {
                        ...
                        try (PreparedStatement ps = displayName == null ? connection.prepareStatement("UPDATE player_levels SET level=?, xp=? WHERE uuid = '*';") : connection.prepareStatement("UPDATE player_levels SET level=?, xp=?, name=?, next_cost=? WHERE uuid = '*';")) {
                            ps.setInt(1, level);
                            ps.setInt(2, xp);
                            if (displayName != null) {
    ...
    public void setLanguage(UUID player, String iso) {
        try {
            ...
            try (Statement statement = connection.createStatement()) {
                try (ResultSet rs = statement.executeQuery("SELECT iso FROM player_language WHERE uuid = '*';")) {
                    if (rs.next()) {
                        try (Statement st = connection.createStatement()) {
                            st.executeUpdate("UPDATE player_language SET iso='*' WHERE uuid = '*';");
                        }
                    } else {
