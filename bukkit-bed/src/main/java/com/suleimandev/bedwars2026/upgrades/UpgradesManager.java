package com.suleimandev.bedwars2026.upgrades;
...
import com.suleimandev.bedwars2026.configuration.UpgradesConfig;
import com.suleimandev.bedwars2026.upgrades.listeners.InventoryListener;
import com.suleimandev.bedwars2026.upgrades.listeners.UpgradeOpenListener;
import com.suleimandev.bedwars2026.upgrades.menu.*;
...
public class UpgradesManager {
    ...
    public static void init() {
        ...
        BukkitBedPlugin.registerEvents(new InventoryListener(), new UpgradeOpenListener());
    }
    ...
    public static boolean loadMenu(String groupName) {
        ...
        BukkitBedPlugin.debug("Registering upgrade menu: " + groupName);
        return true;
    }
    ...
    private static boolean loadCategory(String name) {
        ...
        BukkitBedPlugin.debug("Registering upgrade: " + name);
        return true;
    }
    ...
    private static boolean loadUpgrade(String name) {
        ...
        for (String s : upgrades.getYml().getConfigurationSection(name).getKeys(false)) {
            ...
            if (upgrades.getYml().get(name + "." + s + ".receive") == null) {
                BukkitBedPlugin.debug("Could not load Upgrade " + name + " tier: " + s + ". Receive not set.");
                continue;
            }
            if (upgrades.getYml().get(name + "." + s + ".display-item") == null) {
                BukkitBedPlugin.debug("Could not load Upgrade " + name + " tier: " + s + ". Display item not set.");
                continue;
            }
            if (upgrades.getYml().get(name + "." + s + ".cost") == null) {
                BukkitBedPlugin.debug("Could not load Upgrade " + name + " tier: " + s + ". Cost not set.");
                continue;
            }
            if (upgrades.getYml().get(name + "." + s + ".currency") == null) {
                BukkitBedPlugin.debug("Could not load Upgrade " + name + " tier: " + s + ". Currency not set.");
                continue;
            }
    ...
    private static boolean loadBaseTrap(String name) {
        ...
        if (upgrades.getYml().get(name + ".receive") == null) {
            BukkitBedPlugin.debug("Could not load BaseTrap. Receive not set.");
            return false;
        }
        if (upgrades.getYml().get(name + ".display-item") == null) {
            BukkitBedPlugin.debug("Could not load BaseTrap. Display item not set.");
            return false;
        }
    ...
    public static int getMoney(Player player, Material currency) {
        if (currency == Material.AIR) {
            double amount = BukkitBedPlugin.getEconomy().getMoney(player);
            return amount % 2 == 0 ? (int) amount : (int) (amount - 1);
        }
        return BukkitBedPlugin.getAPI().getShopUtil().calculateMoney(player, currency);
    }
    ...
    public static Material getCurrency(String name) {
        ...
        return BukkitBedPlugin.getAPI().getShopUtil().getCurrency(name);
    }
    ...
    public static void setCustomMenuForArena(IArena arena, UpgradesIndex menu) {
        if (!customMenuForArena.containsKey(arena)) {
            ...
            BukkitBedPlugin.debug("Registering custom menu for arena: " + arena.getArenaName() + ". Using index: " + menu.getName());
        } else {
            BukkitBedPlugin.debug("Overriding custom menu for arena: " + arena.getArenaName() + ". Using index: " + menu.getName() + " Old index: " + customMenuForArena.get(arena).getName());
