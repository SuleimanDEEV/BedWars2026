...
public class HalloweenListener implements Listener {
    ...
    public HalloweenListener() {
        ambienceSound = Sound.valueOf(BukkitBedPlugin.getForCurrentVersion("AMBIENCE_CAVE", "AMBIENT_CAVE", "AMBIENT_CAVE"));
        ghastSound = Sound.valueOf(BukkitBedPlugin.getForCurrentVersion("GHAST_SCREAM2", "ENTITY_GHAST_SCREAM", "ENTITY_GHAST_SCREAM"));
    }
        ...
        if (e.getKiller() != null) {
            ...
            if (location.getBlock().getType() == Material.AIR) {
                ...
                if (!Misc.isBuildProtected(location, e.getArena())) {
                    location.getBlock().setType(Material.valueOf(BukkitBedPlugin.getForCurrentVersion("WEB", "WEB", "COBWEB")));
                    e.getArena().addPlacedBlock(location.getBlock());
                    location.getBlock().setMetadata("give-bw-exp", new FixedMetadataValue(BukkitBedPlugin.plugin, "ok"));
    ...
    public void onJoin(PlayerJoinArenaEvent e) {
        if (!e.isSpectator()) {
            Bukkit.getScheduler().runTaskLater(BukkitBedPlugin.plugin, () -> e.getPlayer().getWorld().playSound(e.getPlayer().getLocation(), ambienceSound, 3f, 1f), 20L);
        }
    }
