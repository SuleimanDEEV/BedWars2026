...
import com.suleimandev.bedwars2026.api.events.shop.ShopBuyEvent;
...
public class CategoryContent implements ICategoryContent {
    ...
    public CategoryContent(String path, String name, String categoryName, YamlConfiguration yml, ShopCategory father) {
        BukkitBedPlugin.debug("Loading CategoryContent " + path);
        ...
        if (yml.get(path + "." + ConfigPath.SHOP_CATEGORY_CONTENT_CONTENT_SLOT) == null) {
            BukkitBedPlugin.plugin.getLogger().severe("Content slot not set at " + path);
            return;
        }

        if (yml.get(path + "." + ConfigPath.SHOP_CATEGORY_CONTENT_CONTENT_TIERS) == null) {
            BukkitBedPlugin.plugin.getLogger().severe("No tiers set for " + path);
            return;
        }
        ...
        if (yml.get(path + "." + ConfigPath.SHOP_CATEGORY_CONTENT_CONTENT_TIERS + ".tier1") == null) {
            BukkitBedPlugin.plugin.getLogger().severe("tier1 not found for " + path);
            return;
        }
    ...
    public static int calculateMoney(Player player, Material currency) {
        if (currency == Material.AIR) {
            return (int) BukkitBedPlugin.getEconomy().getMoney(player);
        }
    ...
    public static void takeMoney(Player player, Material currency, int amount) {
        if (currency == Material.AIR) {
            if (!BukkitBedPlugin.getEconomy().isEconomy()) {
                player.sendMessage("Â§4Â§lERROR: This requires Vault Support! Please install Vault plugin!");
                return;
            }
            BukkitBedPlugin.getEconomy().buyAction(player, amount);
            return;
        }
