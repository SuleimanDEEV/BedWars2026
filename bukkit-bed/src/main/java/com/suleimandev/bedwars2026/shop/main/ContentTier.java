...
public class ContentTier implements IContentTier {
    ...
    public ContentTier(String path, String tierName, String identifier, YamlConfiguration yml) {
        BukkitBedPlugin.debug("Loading content tier" + path);

        if (yml.get(path + ConfigPath.SHOP_CONTENT_TIER_ITEM_MATERIAL) == null) {
            BukkitBedPlugin.plugin.getLogger().severe("tier-item material not set at " + path);
            return;
        }
        ...
        try {
            ...
            BukkitBedPlugin.plugin.getLogger().severe(path + " doesn'*'s not recognized as a tier!");
            return;
        }

        if (yml.get(path + ConfigPath.SHOP_CONTENT_TIER_SETTINGS_COST) == null) {
            BukkitBedPlugin.plugin.getLogger().severe("Cost not set for " + path);
            return;
        }
        ...
        if (yml.get(path + ConfigPath.SHOP_CONTENT_TIER_SETTINGS_CURRENCY) == null) {
            BukkitBedPlugin.plugin.getLogger().severe("Currency not set for " + path);
            return;
        }

        if (yml.getString(path + ConfigPath.SHOP_CONTENT_TIER_SETTINGS_CURRENCY).isEmpty()) {
            BukkitBedPlugin.plugin.getLogger().severe("Invalid currency at " + path);
            return;
        }
        ...
        itemStack = BukkitBedPlugin.nms.createItemStack(yml.getString(path + ConfigPath.SHOP_CONTENT_TIER_ITEM_MATERIAL),
